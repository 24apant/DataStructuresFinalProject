...
class minimaxAlgorithm:
    ...
    def getBestMoveFromTree(self):
        # go to leaves
        # compare all leaves, based on the depth figure out whose move it is
        # if ai's move, pick lowest score, if human's pick highest, set parent's score to min of child's score
        # keep going until root and location will be tracked as well
        return self.__getBestMoveRec(self.root,0)

    def __getBestMoveRec(self, current, depth):
        if not isinstance(current, Node):
            return
        #gets node with highest score over other nodes
        if(depth % 2 == 0): #human
            current.score = self.__getBestHumanChild(current)
        elif(depth % 2 == 1):
            current.score = self.__getBestAIChild(current)
        for i in range(NUM_CELLS):
            self.__getBestMoveRec(current.children[i],depth+1)

        return self.__getBestAIChild(current)[1]

        #get the position of board with best score, either for ai or against
